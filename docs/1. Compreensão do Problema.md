...
- **Modelagem Matemática de Geometria Contínua (`core/physics.py`):** Codificou o cálculo exato de distância Euclidiana e a verificação precisa de colisão com o Sol central baseada na distância de um ponto a um segmento de reta, otimizando o `SUN_RADIUS` para `10.5` com base em auditoria de logs.
- **Prevenção de Colisão Física com Obstáculos (Ray-Casting):** Implementou a detecção de obstrução retilínea entre naves em trânsito e planetas estáticos/orbitais intermediários (`intersects_planet`), abortando lançamentos autodestrutivos.
