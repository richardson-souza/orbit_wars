...
2. **Projeção Orbital Dinâmica:** A função `get_planet_position_at_step` (`core/physics.py`) reconstrói a posição de rotação circular uniforme dos planetas internos com base no ângulo inicial calculado via `math.atan2` e na velocidade angular do ambiente.
3. **Detecção e Prevenção de Colisão Solar via Ray-Casting:** A função `intersects_sun` (`core/physics.py`) utiliza o cálculo de projeção matemática para encontrar a menor distância ortogonal de um ponto a um segmento de reta (vetor de trajetória), descartando imediatamente qualquer jogada cujo trajeto passe a menos de 10.0 unidades de distância do Sol central `(50, 50)`.